...
*   **Ứng dụng:** Cohen's Kappa dùng để hiệu chuẩn độ tin cậy của Multi-Judge Consensus Engine. Nếu hệ số \(\kappa \ge 0.61\), độ đồng thuận được coi là tốt và điểm số đáng tin cậy. Nếu thấp hơn, ở hướng mở rộng sản phẩm, hệ thống có thể kích hoạt thêm logic trọng tài hoặc xem xét lại Rubric chấm điểm của prompt. Trong bản nộp hiện tại, hệ thống ưu tiên offline deterministic judge để chạy ổn định.
...
*   Khi đánh giá hàng ngàn test cases, nếu sử dụng các mô hình LLM cao cấp làm Judge thật thì chi phí và thời gian sẽ tăng cao. Vì vậy bản hiện tại dùng offline deterministic judge để đảm bảo benchmark chạy ổn định khi không có API key.
*   **Chiến lược tối ưu hóa (Hybrid Judge):**
    *   Sử dụng offline judge để thực hiện đánh giá sơ bộ đầu tiên trong môi trường lab.
    ...
    *   Nếu mở rộng sang sản phẩm thật, chỉ các case không chắc chắn hoặc có xung đột cao mới nên được chuyển lên real LLM judge. Cách tiếp cận này giúp giảm khoảng 30% - 50% chi phí mà không làm giảm độ chính xác chung của hệ thống.
